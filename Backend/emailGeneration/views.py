...
def person_detail(request,id):
  try:
    person = personData.objects.get(pk=id)
     ...
     return Response({"error":{"msg":"User does not exist"},"result":{},"success":False ,"status":status.HTTP_404_NOT_FOUND})
...
def EmailGeneration(request , id):
  try:
        ...
        if openAiSerializer.is_valid():
            ...
            return JsonResponse({'response': finalResponse,"success":True,"error":{},"status":status.HTTP_200_OK})
        
        else:
          return Response({"error":openAiSerializer.errors,"success":False,"result":{}, "status":status.HTTP_400_BAD_REQUEST}) 
  except personData.DoesNotExist:
    return Response({"error":{"msg":"User does not exist"},"result":{},"success":False ,"status":status.HTTP_404_NOT_FOUND})
  
  except openai.error.AuthenticationError as e:
    return Response({"error":{"msg":"Incorrect Api Key provided"},"result":{},"success":False ,"status":status.HTTP_404_NOT_FOUND})      
  
  except openai.error.ServiceUnavailableError or openai.error.RateLimitError or openai.error.APIError as e:
      return Response({"error":{"msg":"Service unavailable! Try after some time"},"result":{},"success":False ,"status":status.HTTP_404_NOT_FOUND})      
